...
def curve_fit_data(xdata, ydata, fit_type, override=False, 
    ...
    def model_function_sine_offset(theta, phi, a, b):
        return a*np.sin((theta+phi)) + b
    
    def model_function_quadratic(x, a, b, c):
        return a*(x**2) + b*(x) + c
    
    def model_function_sqrt_int(x, a, c):
    ...
    model_functions = {
        ...
        'log' : model_function_log,
        'exp' : model_function_exp,
        'sin-off': model_function_sine_offset,
        'quad' : model_function_quadratic,
        'sqrt' : model_function_sqrt,
        'sqrt-int' : model_function_sqrt_int,
        'custom' : model_function_custom,
        }
    ...
    if not override:
        new_xdata = np.linspace(min(xdata), max(xdata), num=1000)
        ...
        if guess is not None:
            if bounds is not None:
                popt, pcov = curve_fit(model_func, xdata, ydata, sigma=uncertainty, 
                                   maxfev=15000, absolute_sigma=abs_sig, p0=guess,
                                   bounds=bounds)
            else:
                popt, pcov = curve_fit(model_func, xdata, ydata, sigma=uncertainty, 
                                   maxfev=15000, absolute_sigma=abs_sig, p0=guess)
            ...
            popt, pcov = curve_fit(model_func, xdata, ydata, sigma=uncertainty, 
                               maxfev=15000, absolute_sigma=abs_sig)
        param_num = len(popt)
...
def quick_plot_residuals(xdata, ydata, plot_x, plot_y,
                         ...
                         uncertainty_x=[], mcolor='red', custom_res=None, 
                         lims=None):
    """
    ...
    """
    if custom_res!=None:
        fig = plt.figure(figsize=custom_res)
        gs = gridspec.GridSpec(ncols=11, nrows=8, figure=fig)
        main_fig = fig.add_subplot(gs[:5,:])
    ...
    #major_ticks = np.arange(min(plot_x), max(plot_x), 20)
    #minor_ticks = np.arange(min(plot_x), max(plot_x), 5)
    
    #main_fig.set_xticks(major_ticks)
    #main_fig.set_xticks(minor_ticks, minor=True)
    #main_fig.set_yticks(major_ticks)
    #main_fig.set_yticks(minor_ticks, minor=True)
    #main_fig.grid(which='minor', alpha=0.5)
    #main_fig.grid(which='major', alpha=0.7)
    ...
    main_fig.set_title(meta['title'], fontsize = 35, pad=30)
    
    if lims!=None:
        uncertainty = [lims*i for i in uncertainty]
    
    if len(uncertainty_x)==0:
        main_fig.errorbar(xdata, ydata, yerr=uncertainty, #xerr=uncertainty_x,
                      markersize='4', fmt='o', color=mcolor, 
